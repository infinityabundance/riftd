...
sealed class RiftEvent {
    ...
    data class IncomingCall(val sessionId: String, val from: String, val rndzvSrtUri: String?) : RiftEvent()
    data class CallStateChanged(val sessionId: String, val state: String) : RiftEvent()
    data class AudioLevel(val peerId: String, val level: Float) : RiftEvent()
