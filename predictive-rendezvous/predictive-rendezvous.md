...
This document provides a conceptual and architectural overview of Predictive Rendezvous as used in riftd. It explains the underlying model, terminology, and design intent so that developers can understand what the system is doing and why, independently of any specific implementation.
...
UDP path established
        ↓
Existing riftd session + media layers


PR operates above transport and below session/media, supplying deterministic parameters (timing, ports, behavior) that allow peers to establish a reachable path. Once a path exists, riftd’s normal encryption, media, and session handling apply.
...
SRTs are exchanged out of band (chat messages, email, QR codes, documents, physical transfer) and can be reconstructed independently by all participants.
...
A peer generates an SRT describing a future rendezvous.

The SRT is shared out-of-band.

At the agreed time window, all peers activate a rendezvous runner.

Each peer independently computes slot schedules.

Probes are emitted according to the schedule.

When convergence occurs, a direct path is established.

riftd upgrades the path to a secure session and media flow.
